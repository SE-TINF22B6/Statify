...
##### 3.3.1 Security
**Goal:** Ensure the confidentiality of user data during the connection to their Spotify account by minimizing data storage and securely managing the access token.
**Tactics:** 
> Redirect users to Spotify’s login page for authentication, ensuring that sensitive credentials are not handled by the web application.
> Store only the access token generated by Spotify after user authentication.
>  Avoid storing any additional sensitive user data to reduce the risk of data breaches and ensure compliance with data protection principles.
##### 3.3.2 Performance
**Goal:** Ensure that users can generate statistics within 5 minutes of account creation.
**Tactics:** 
> Use Spotify Authentification and fetch user's profile data instead of creating a new account from scratch.
> Optimize queries to the Spotify API to reduce latency in fetching listening history.
##### 3.3.3 Modifiability
**Goal:** Ensure that changes can be made easily by reducing the interdependence of components.
**Tactics:** 
> Design the system in a modular way, with clearly defined interfaces between components.
> Define abstract classes to decouple components from specific implementations.
> Use version control systems (e.g., Git) to manage changes in the codebase.
...
##### 4.1 Spotify API Usage:
> The application must comply with Spotify's API usage policies and rate limits.
> The application should handle token expiration and refreshing as specified by Spotify's OAuth 2.0 protocol.
##### 4.2 Database:
> Use a relational database (MySQL) for storing user access tokens and statistics.
##### 4.3 Database Server Hostage:
> The Database server must be hosted on a cloud-based server, we use BWCloud, since its capacity is enough for our purpose and it's free of charge.
##### 4.4 Development Tools:
> Dependency on version control systems (e.g., Git) for codebase management.
